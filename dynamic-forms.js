          ...
          (attrs.template ? $q.when($parse(attrs.template)($scope)) :
            ...
            buildFields = function (field, id) {
              if (String(id).charAt(0) == '$') {
                ...
                if (angular.isDefined(field.label.text)) {angular.element(newElement).html(field.label.text);}
                ...
                if (field.type === 'number' || field.type === 'range') {
                  ...
                  if (angular.isDefined(field.label.text)) {newElement.attr('alt', field.label.text);}
                ...
                if (angular.isDefined(field.label.text)) {
                  ...
                  //  Fieldset elements put their labels in legend child elements.
                    ...
                    newElement.prepend(angular.element($document[0].createElement('legend')).html(field.label.text));
                  }
                  //  Button elements get their labels from their contents.
                  else if (["button", "legend", "reset", "submit"].indexOf(field.type) > -1) {
                    newElement.html(field.label.text);
                  }
                  //  Everything else should be wrapped in a label tag.
					...
					//wraping dive element over input
					if(angular.isDefined(field.divClassName)){
						newElement.attr('class', field.divClassName);
					}
					else{
						newElement.attr('class', "col-sm-10");
					}
					//wratping div element over both
					newElement = newElement.wrap('<div></div>').parent();
					//crate labele element
					var labelElement = angular.element($document[0].createElement('label'));
					if(angular.isDefined(field.label.className)){
						labelElement.attr('class', field.label.className);
					}
					else{
